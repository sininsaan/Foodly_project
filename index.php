<?php
session_start();
if(isset($_SESSION['roll_no'])) { // CHANGED: redirect if already logged in
    header("Location: dashboard.php");
    exit();
}
$error = isset($_GET['error']) ? $_GET['error'] : ''; // NEW: error param from login_process.php
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        /* LEFT PANEL — always fixed height, icon always centered */
        .login-left {
        ...
        /* LEFT PANEL brand — fixed size, never shrinks */
        .left-brand-wrap {
            display: flex;
            align-items: center;
            justify-content: center;
        ...
        /* RIGHT PANEL — scrollable if content overflows */
        .login-right {
            opacity: 0;
...
<div class="login-page" id="login-page">
    ...
    <div class="login-right">
        <div class="login-form-box">
            <p class="login-form-subtitle">Welcome back 👋</p>
            ...
            <div class="alert-error">❌ Invalid roll number or password. Try again.</div>
            <?php elseif($error === 'registered'): ?>
            <div class="alert-error" style="background:#e8f5ee;border-color:#6ee7b7;color:#065f46;">✅ Registered! Please login.</div>
            <?php endif; ?>
    ...
    <div style="position:relative;">
        ...
        <button type="button" onclick="togglePass('reg-password', this)" style="position:absolute;right:14px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:var(--text-muted);">
            ...
            <p style="font-size:11px;color:var(--text-muted);margin-top:32px;text-align:center;">
                Foodly v1.0
            </p>
        </div>
    </div>
...
// intro hata, phir 300ms baad right side fade in
setTimeout(() => {
