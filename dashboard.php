...
// Auth check - comment out for frontend testing
...
<head>
    ...
    <style>
        ...
        /* Card qty controls — shown when item in cart */
        .card-qty-controls {
            display: none;
            align-items: center;
            gap: 8px;
        ...
        <div class="food-card animate-in <?php echo !$row['is_available'] ? 'out-of-stock' : ''; ?>" data-name="<?php echo strtolower($name); ?>" data-category="<?php echo strtolower($cat); ?>" id="card-<?php echo $id; ?>">
            ...
            <div class="food-card-body">
                ...
                <div class="food-card-footer">
                    ...
                    <div>
                        <!-- Add button — hidden when item in cart -->
                        ...
                        <!-- Qty controls — shown when item in cart -->
        ...
        <div id="slots-section" style="margin-top:24px;display:none;">
            ...
            <div class="timeslot-grid" id="timeslot-grid">
                <?php foreach($slots as $i => $slot):
                    $full = ($slot['current_count'] >= $slot['max_capacity']);
                    $label = $slot['label'] ?? ($slot['start_time'].' - '.$slot['end_time']);
                    $remaining = $slot['max_capacity'] - $slot['current_count'];
                ?>
                <div class="timeslot-option <?php echo $full ? 'disabled' : ''; ?> <?php echo (!$full && $i===0) ? 'selected' : ''; ?>"
                     onclick="<?php echo $full ? '' : "selectSlot(this, {$slot['id']})"; ?>"
                    ...
                    <?php if($full): ?>
                    <span class="timeslot-status status-full">Full (<?php echo $slot['current_count'].'/'.$slot['max_capacity']; ?>)</span>
                    <?php else: ?>
...
<div class="timer-modal" id="timer-modal">
    <div class="timer-box">
        ...
        <p style="font-size:13px;color:var(--text-muted);margin-bottom:24px;">Review your items. Order will auto-confirm in <strong>15 seconds</strong>.</p>
...
// Update card UI when qty changes
...
function updateTimerDisplay() {
    ...
    const offset = circumference - (timerSec / 15) * circumference;
