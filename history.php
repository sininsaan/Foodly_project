...
.bar-tracker .dot.is-ready {
    background: #2d9a5f !important;
    color: white !important;
    border-color: #2d9a5f !important;
}

.bar-tracker .dot.is-ready::after {
    content: '✓';
    font-weight: bold;
}
...
<div style="margin-bottom:28px;">
    ...
    ?>
    <div class="order-card animate-in" data-order-id="<?php echo $ao['id']; ?>" 
    data-current-status="<?php echo $ao['status']; ?>" 
    style="background:var(--bg-card);border-radius:var(--radius);border:1px solid var(--border);padding:24px;margin-bottom:16px;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px;">
            ...
            <span class="badge <?php echo $badge; ?> order-status-badge"><?php echo $ao['status']; ?></span>
        </div>
        ...
        <div class="order-msg" style="border-radius:10px;padding:12px 16px;font-size:14px;font-weight:600;<?php echo $msg[1]; ?>">
            <?php echo $msg[0]; ?>
        </div>
    </div>
    <?php endforeach; ?>
        ...
        <table class="data-table">
            ...
            <tbody>
                <?php foreach($display_orders as $order):
                    $badge = ['Completed'=>'badge-completed','Missed'=>'badge-missed','Cancelled'=>'badge-cancelled'][$order['status']] ?? 'badge-completed';
                ?>
                <tr>
...
function updateOrderCardUI(order) {
    ...
    if(badge) {
        const badgeMap = { 
            ...
            'Ready':'badge-ready', 'Completed':'badge-completed', 'Missed':'badge-missed' 
        };
        badge.className = 'badge order-status-badge ' + (badgeMap[order.status] || 'badge-pending');
        badge.textContent = order.status;
    }
    
    // Tracker Dots Update
    card.querySelectorAll('.tracker-step').forEach((step, i) => {
        ...
        if(i < currentIdx) {
            ...
            if(dot) dot.textContent = (order.status === 'Ready' || order.status === 'Completed') ? '✓' : '';
        }
    });
}
</script>
